...
PDF.new_LorenzoTT = function()
    ...
    local write_indirect_object2
         
    write_object = function(fh, obj)
        if type(obj) == "table" and obj.datatype == "stream" 
            then
    ...
    write_direct_object = function(fh, obj)
        if type(obj) ~= "table" 
            then
            fh:write(obj .. "\n")
        elseif obj.datatype == "dictionary" 
            then
            local k, v
            fh:write("<<\n")
            for k, v in pairs(obj.contents) 
                do
                fh:write(string.format("/%s ", k))
                write_object(fh, v)
            end
            fh:write(">>\n")
        elseif obj.datatype == "array" 
            then
            local v
            fh:write("[\n")
            for _, v in ipairs(obj.contents) 
                do
                write_object(fh, v)
            end
            fh:write("]\n")
        elseif obj.datatype == "imageObj" then
            local k, v
            fh:write("<<\n")
            for k, v in pairs(obj.contents) 
                do
            ...
            fh:write(">>\n")
            fh:write("stream\n")
            fh:write(obj.streamData)
            fh:write("\n")
            fh:write("endstream\n")
        elseif obj.datatype == "stream" 
            then
            local len = 0    
            if type(obj.contents) == "string" 
                then
    ...
    local write_xref_table = function(fh)
        ...
        for i, obj in ipairs(object) 
            do
            fh:write(string.format("%010d %05d n \n", obj.offset, 0))
        end
    end
    ...
    pdf.new_page = function(pdf, pgnum, pdf_img_W, pdf_img_H)
        ...
        pg.set_font = function(pg, font_obj, size)
            table.insert(contents, string.format("%s %f Tf", use_font(font_obj), size))
        end
         
        pg.set_text_pos = function(pg, x, y)
            table.insert(contents, string.format("%f %f Td", x, y))
        end
         
        pg.show = function(pg, str)
            table.insert(contents,string.format("(%s) Tj", str))
        end
        ...
        pg.set_char_spacing = function(pg, spc)
            table.insert(contents,string.format("%f Tc", spc))
        end
        ...
        pg.moveto = function(pg, x, y)
            table.insert(contents,string.format("%f %f m", x, y))
        end
         
        pg.lineto = function(pg, x, y)
            table.insert(contents,string.format("%f %f l", x, y))
        end

        pg.linewidth = function(pg, x)
            table.insert(contents,string.format("%f w", x))
        end
        ...
        pg.curveto = function(pg, x1, y1, x2, y2, x3, y3)
            ...
            if x3 and y3 
                then
                str = string.format("%f %f %f %f %f %f c",x1, y1, x2, y2, x3, y3)
            else
                str = string.format("%f %f %f %f v",x1, y1, x2, y2)
            end
            table.insert(contents, str)
        end
         
        pg.rectangle = function(pg, x, y, w, h)
            table.insert(contents,string.format("%f %f %f %f re",x, y, w, h))
        end
        ...
        pg.setgray = function(pg, which, gray)
            ...
            if which == "fill" 
                then
                table.insert(contents, string.format("%d g", gray))
            else
                table.insert(contents,string.format("%d G", gray))
            end
        end
        ...
        pg.setrgbcolor = function(pg, which, r, g, b)
            ...
            if which == "fill" 
                then
                table.insert(contents,string.format("%f %f %f rg", r, g, b))
            else
                table.insert(contents,string.format("%f %f %f RG", r, g, b))
            end
        end
        ...
        pg.setcmykcolor = function(pg, which, c, m, y, k)
            ...
            if which == "fill" 
                then
                table.insert(contents,string.format("%f %f %f %f k", c, m, y, k))
            else
                table.insert(contents,string.format("%f %f %f %f K", c, m, y, k))
            end
        end
        ...
        pg.setflat = function(pg, i)
            ...
            table.insert(contents,string.format("%d i", i))
        end
         
        pg.setlinecap = function(pg, j)
            assert(j == 0 or j == 1 or j == 2)
            table.insert(contents,string.format("%d J", j))
        end
        ...
        pg.setlinewidth = function(pg, w)
            table.insert(contents,string.format("%d w", w))
        end
         
        pg.setmiterlimit = function(pg, m)
            assert(m >= 1)
            table.insert(contents,string.format("%d M", m))
        end
        ...
        pg.setdash = function(pg, array, phase)
            ...
            table.insert(contents,string.format("[%s] %d d", str, phase))
        end
        ...
        pg.ImageCm = function(pg, w, h, x, y)
            table.insert(contents, tostring(w) .. " 0 0 " .. tostring(h) .. " " .. tostring(x) .. " " .. tostring(y) .. " cm")
        end
             
        pg.ImageDo = function(pg)
            table.insert(contents,"/Im" .. pgnum .. " Do")
        end
        ...
        pg.transform = function(pg, a, b, c, d, e, f) -- aka concat
            table.insert(contents,string.format("%f %f %f %f %f %f cm",a, b, c, d, e, f))    
        end
        ...
        pg.add = function(pg)
            local contents_obj, this_obj, resources, image_stream_obj, image_obj
            local i, font_obj
            image_obj = add {
                datatype = "imageObj",
                streamData = jpgstream,
                contents =      {
                    Type = "/XObject",
                    Subtype = "/Image",
                    Height = pdf_img_H,
            ...
            resources = {
                datatype = "dictionary",
                contents = {
                    XObject = "<< " .. "/Im" .. tostring(pgnum) .. " " .. get_ref(image_obj) .. " >>",
                    Font =      {
                        datatype = "dictionary",
                        contents = {}
                                },
            ...
            for i, font_obj in ipairs(used_font) 
                do
            ...
            table.insert(pages_obj.contents.Kids.contents,get_ref(this_obj))
