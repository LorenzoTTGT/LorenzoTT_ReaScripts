...
local function gorenderst()
    ...
    if (sel_WAVformat[4] == true) 
        then _,_,_ = ultraschall.RenderProject_RenderTable(nil, OutputWAV4824, false, true, false)
               ...
               do
                ...
                local TrackISRC = ""
                         for marker_1 = 1, mrkrsNumb
                           do 
                             local markcount_i_name = allREGmarkersarray[marker_1][1]
                             --reaper.ShowConsoleMsg(tostring(" name:"..allREGmarkersarray[marker_1][1]))
                             local markcount_i_name = tostring(markcount_i_name)
                             ...
                             --if markcount_i_name DELETE #MARKER -----
                              ...
                              if (string.sub(allREGmarkersarray[marker_1][1], 1, 4) == "ISRC") or (string.sub(allREGmarkersarray[marker_1][1], 1, 4) == "isrc")
                                then 
                                  TrackISRC = markcount_i_name:gsub("ISRC=", "")
                                  TrackISRC = TrackISRC:gsub("isrc=", "")
            ...
            page:end_text()
             ...
             if  (i_rndTrack == allRegNmbr) and (interLineSpace < 50) then
                        if previous_i_interline ~= nil 
                          ...
                          else previous_i_Endline = previous_i_rndTitleLine
                        end
